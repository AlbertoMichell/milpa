 ...
 */
export async function updateBlueGreenConfig(iaUrl: string) {
  try {
 ...
 */
export function shouldRouteToV2(sessionId: string): boolean {
  if (!config.v2Enabled) {
    return false;
  }
 ...
 */
export function blueGreenMiddleware(app: any) {
 ...
 */
export function getBlueGreenConfig(): BlueGreenConfig {
  return { ...config };
}
