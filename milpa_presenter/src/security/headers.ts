...
async function headersPlugin(app: any) {
  app.addHook("onSend", async (_req: any, reply: any, payload: any) => {
    ...
    reply.header(
      ...
      "default-src 'none'; script-src 'unsafe-inline'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"
    );
    return payload;
  });
}
