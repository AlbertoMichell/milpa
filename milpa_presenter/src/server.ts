...
const circuit = new CircuitBreaker(
  ...
  30,
  config.CIRCUIT_MAX_P95_MS
);

/** Rutas al backend que pueden tardar minutos (indexado, RAG, ingesta). */
function upstreamTimeoutMsForSuffix(suffix: string): number {
  const path = suffix.split("?")[0].toLowerCase();
  if (
    path.includes("api/documents/ingest") ||
...
app.addContentTypeParser(
  ...
  async (_req: any, payload: NodeJS.ReadableStream) => {
...
app.get("/favicon.ico", async (_req: any, rep: any) => {
  return rep.code(204).send();
});

// Redirect /ui/ -> /ui/checks (landing page)
app.get("/ui", async (_req: any, rep: any) => { rep.redirect("/ui/checks", 301); });
app.get("/ui/", async (_req: any, rep: any) => { rep.redirect("/ui/checks", 301); });

// Ruta demo: devuelve HTML con enlaces internos/externos para verificar sanitización
app.get("/demo/sanitized", async (_req: any, rep: any) => {
  const html = `
...
app.get("/doc/:id", async (req: any, rep: any) => {
...
app.get("/ai/health", async (_req: any, rep: any) => {
  try {
...
app.get('/api/admin/users', async (_req: any, rep: any) => {
  try {
    const url = new URL('/admin/users', config.IA_URL).toString();
    const res = await fetch(url);
    if (!res.ok) { rep.code(res.status); return { error: 'backend error' }; }
    return res.json();
  } catch (e: any) {
    rep.code(502);
...
app.all('/ai/*', async (req: any, rep: any) => {
  try {
    proxyTotal.inc();
    ...
    for (const [k, v] of Object.entries(req.headers || {})) {
      if (typeof v === 'string') fHeaders[k] = v;
    }
    ...
    if (method !== 'GET' && method !== 'HEAD' && Buffer.isBuffer(body)) {
      fHeaders['content-length'] = String((body as Buffer).length);
    ...
    try {
      const buffer = await scheduler.enqueue(async () => {
        const res = await fetch(target, { method, headers: fHeaders, body, signal: controller.signal as any });
        // Copiar estado y headers
        rep.code(res.status);
        res.headers.forEach((val, key) => {
          const kl = key.toLowerCase();
          if (kl === 'content-length' || kl === 'content-encoding') return;
          rep.header(key, val);
        });
...
app.post("/sanitize/preview", async (req: any, rep: any) => {
...
app.get("/ui/checks", async (_req: any, rep: any) => {
...
app.get('/ui/library', async (_req: any, rep: any) => {
  const page = `<!doctype html>
  <html lang="es">
  <head>
    <meta charset="utf-8" />
...
app.get('/ui/library/:docId', async (req: any, rep: any) => {
...
app.get('/ui/query', async (_req: any, rep: any) => {
  const page = `<!doctype html>
  <html lang="es">
  <head>
    <meta charset="utf-8" />
...
// Estado de ejecución
app.get('/runtime/status', async (_req: any, rep: any) => {
  const stats = scheduler.stats;
  const circuitState = (circuit as any).stats?.state ?? 'closed';
  const payload = {
...
// Ruta: /ui/ingesta
// ─────────────────────────────────────────────────────────────────────────────
app.get('/ui/ingesta', async (_req: any, rep: any) => {
