...
async function headersPlugin(app) {
    ...
    app.addHook("onSend", async (_req, reply, payload) => {
        ...
        reply.header("Content-Security-Policy", `default-src 'none'; script-src 'unsafe-inline'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; font-src 'self'; connect-src ${connectSrc}; frame-ancestors 'none'; base-uri 'none'; form-action 'self'`);
        return payload;
    });
}
